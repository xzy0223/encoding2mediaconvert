...
Uses ffmpeg-python library instead of direct command line calls.
"""
...
class VideoAnalyzer:
    ...
    using Claude 3.5 on Bedrock. Uses ffmpeg-python library.
    """
    ...
    def extract_video_info(self, s3_path: str) -> Dict[str, Any]:
        """
        Extract video information using ffmpeg-python from a video file in S3.
        ...
        with tempfile.NamedTemporaryFile(suffix=os.path.splitext(object_key)[1]) as temp_file:
            ...
            try:
                ...
                probe_result = ffmpeg.probe(temp_file.name)
                ...
                raise RuntimeError(f"Failed to extract video information: {e.stderr.decode()}")

    def _extract_frame_info(self, video_path: str) -> Dict[str, Any]:
        """
        Extract detailed frame information from the video using ffmpeg-python.
        ...
        """
        try:
            # Use ffmpeg.probe with select_streams to get frame information
            frame_info = ffmpeg.probe(
                video_path,
                select_streams='v:0',
                show_frames=None
            )
            
            # Process frame data to extract key information
            processed_frames = []
            for frame in frame_info.get('frames', []):
                processed_frames.append({
                    'pkt_pts_time': frame.get('pkt_pts_time'),
            ...
            raise RuntimeError(f"Failed to extract frame information: {e.stderr.decode()}")
